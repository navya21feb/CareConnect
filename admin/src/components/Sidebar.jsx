  ...
  return (
    <div className='min-h-screen bg-white border-r'>
      ...
      {dToken && <ul className='text-[#515151] mt-5'>
        ...
        <NavLink to={'/doctor-prescription'} className={({ isActive }) => `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? 'bg-[#F2F3FF] border-r-4 border-primary' : ''}`}>
          <img className='min-w-5' src={assets.people_icon} alt='' />
          <p className='hidden md:block'>PrescriptionGenerator</p>
        </NavLink>
        <NavLink to={'/health-report'} className={({ isActive }) => `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? 'bg-[#F2F3FF] border-r-4 border-primary' : ''}`}>
          <img className='min-w-5' src={assets.people_icon} alt='' />
          <p className='hidden md:block'>HealthReportViewer</p>
        </NavLink>
        <NavLink to={'/video-call'} className={({ isActive }) => `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? 'bg-[#F2F3FF] border-r-4 border-primary' : ''}`}>
          <img className='min-w-5' src={assets.people_icon} alt='' />
          <p className='hidden md:block'>Video Call</p>
        </NavLink>
      </ul>}
    </div>
  )
